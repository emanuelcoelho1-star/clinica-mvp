...
const FORM_SECTIONS = [
  {
    ...
    fields: [
      { name: "procedimento", label: "Procedimento realizado", type: "text", placeholder: "Ex: Restauração, Limpeza, Exodontia..." },
  ...
  },
  {
    ...
    fields: [
      { name: "descricao", label: "Descrição detalhada", type: "textarea", placeholder: "Descreva o procedimento realizado, materiais utilizados, técnica..." },
    ],
  },
  {
    ...
    fields: [
      ...
      { name: "observacoes", label: "Observações", type: "textarea", placeholder: "Notas adicionais, retorno, cuidados pós..." },
    ],
  },
];
...
/* ═══════════════════════════════════════════════════════════
   COMPONENTE
   ═══════════════════════════════════════════════════════════ */
function AbaEvolucoes({ pacienteId }) {
  ...
  async function handleSalvar(e) {
    ...
    try {
      ...
      setErroForm("Não foi possível salvar a evolução.");
    } finally {
      setSalvando(false);
    }
  }
  ...
  async function handleExcluir() {
    ...
    try {
      ...
      setErro("Erro ao excluir evolução.");
    } finally {
      setExcluindo(false);
    }
  }
  ...
  if (tela === "lista") {
    return (
      <div style={S.wrapper}>
        ...
        <div style={S.header}>
          <div>
            <h2 style={S.title}>Evoluções</h2>
            <p style={S.subtitle}>
              {evolucoes.length} registro{evolucoes.length !== 1 ? "s" : ""} clínico{evolucoes.length !== 1 ? "s" : ""}
            </p>
          </div>
          <button
            style={{
        ...
        {carregando && (
          <div style={S.feedbackBox}>
            ...
            <span style={S.feedbackText}>Carregando evoluções...</span>
          </div>
        )}
        ...
        {/* Vazio */}
        {!carregando && !erro && evolucoes.length === 0 && (
          <div style={S.emptyBox}>
            {Icons.activity}
            <h3 style={S.emptyTitle}>Nenhuma evolução registrada</h3>
            <p style={S.emptyText}>Registre a primeira evolução deste paciente para acompanhar os procedimentos realizados.</p>
            <button
              style={{
                ...S.btnPrimary,
                marginTop: "4px",
        ...
        {confirmarExclusao && (
          <div style={S.overlay} onClick={() => setConfirmarExclusao(null)}>
            <div style={S.modal} onClick={(e) => e.stopPropagation()}>
              ...
              <h3 style={S.confirmTitle}>Excluir evolução #{confirmarExclusao.id}?</h3>
              <p style={S.confirmText}>Esta ação é irreversível. Todos os dados desta evolução serão permanentemente removidos.</p>
        ...
        {/* Keyframes */}
        <style>{`
          @keyframes evolucao-pulse-dot {
            0%, 80%, 100% { transform: scale(0); opacity: 0.5; }
            40% { transform: scale(1); opacity: 1; }
          }
          @keyframes evolucao-fade-in {
      ...
      <div style={S.wrapper}>
        ...
        <div style={S.header}>
          <div>
            ...
            <h2 style={S.title}>{editando ? "Editar evolução" : "Nova evolução"}</h2>
            <p style={S.subtitle}>{editando ? "Altere os dados da evolução clínica." : "Registre um novo procedimento realizado."}</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSalvar} style={S.formWrap}>
          ...
          <div style={S.formCard}>
            <div style={S.formCardHeader}>
              ...
              <span style={S.formCardTitle}>Data da evolução</span>
            </div>
            <input
              type="date"
              name="data"
      ...
      <div style={S.wrapper}>
        ...
        <div style={S.header}>
          <div>
            ...
            <h2 style={S.title}>Evolução #{detalhes.id}</h2>
        ...
        {/* Pills resumo */}
        {(detalhes.procedimento || detalhes.dente) && (
          <div style={S.pillRow}>
...
const S = {
  wrapper: {
    ...
    gap: "24px",
    animation: "evolucao-fade-in 0.3s ease",
  },
  ...
  loadingDot1: {
    ...
    animation: "evolucao-pulse-dot 1.4s ease-in-out infinite", animationDelay: "0s",
  },
  loadingDot2: {
    width: "10px", height: "10px", borderRadius: "50%", background: "#2563eb",
    animation: "evolucao-pulse-dot 1.4s ease-in-out infinite", animationDelay: "0.2s",
  },
  loadingDot3: {
    width: "10px", height: "10px", borderRadius: "50%", background: "#2563eb",
    animation: "evolucao-pulse-dot 1.4s ease-in-out infinite", animationDelay: "0.4s",
  },
  ...
  modal: {
    ...
    animation: "evolucao-fade-in 0.2s ease",
  },
